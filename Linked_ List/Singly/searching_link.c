...
void searching(int data){
    ...
    if(head->info == data)
        printf("The Element is in linked list");
    while(head !=NULL)
    {
        head = head->link;
        if(head->info == data)
...
int main(){
    ...
    scanf("%d",&data);
    add_beg(&global,data);
    printf("\nInsert the value for last node : \n");
    scanf("%d",&data2);
    add_end(&global,data2);
    printf("\nInsert the value you want to add :\n");
    scanf("%d",&data3);
    printf("\nInsert the position :\n");
    scanf("%d",&j);
    ...
    scanf("%d",&d);
    searching(d);
    // ( e == 1)?printf("The Element is is linked list"):printf("404 Not Found !!");
    return 0;
}